...
image: blog/build-dev-to-clone-with-nuxt-new-fetch/main.png
...
This article is intended to demonstrate use cases and awesomeness of new NuxtJS `fetch` functionality [introduced in release v2.12](https://nuxtjs.org/docs/2.x/components-glossary/pages-fetch#nuxt-gt-2-12), and show you how to apply its power in your own projects. For in-depth technical analysis and details of the new `fetch` you can check [Krutie Patel’s article](/understanding-how-fetch-works-in-nuxt-2-12).
...
Now we finally can start developing our DEV.TO clone powered by Nuxt and [new fetch](https://nuxtjs.org/docs/2.x/components-glossary/pages-fetch).

### URL structure

Let’s imitate DEV.TO URL structure for our simple app. Our [pages](https://nuxtjs.org/docs/2.x/concepts/views#pages) folder should look like this:
...
Also notice that the new `fetch` hook doesn’t serve just to dispatch Vuex store action or committing mutation to set state, now it has access to `this` context, and is able to mutate component’s data directly. This is a very important new feature, and you can [read more](/understanding-how-fetch-works-in-nuxt-2-12) about it in the previous article about `fetch`.
...
- Read [Krutie Patel article](/understanding-how-fetch-works-in-nuxt-2-12) with in-depth analysis of how new `fetch` hook works
